...
class Checkout extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      UIData: {
        /* 支付方式 */
        // 1:信用卡 2:LINE PAY 3:遠傳電信帳單 4:APPLY PAY 5aftee
        use_method: 0,
  ...
  sendCheckOut = () => {
    ...
    if (use_method === 1) {
      ...
      Helper.axios.fetch(
        ...
        (cb) => {
          ...
          } else if (redirectPage === 'MRAPP') {
            let url = '';
            if (window.location.hostname === 'www.ezding.com.tw') {
              url = encodeURI(
                `https://static.oopocket.com/3rd/ezding/stable/index.html?result=SUCCESS&order_id=${orderID}`,
              );
            } else {
              url = encodeURI(
                `https://at-uat-static.oopocket-dev.com/3rd/ezding/latest/index.html?result=SUCCESS&order_id=${orderID}`,
              );
            }
        ...
        {
          errorFn: () => {
            if (localStorage.getItem('redirectPage') === 'MRAPP') {
              let url = '';
              if (window.location.hostname === 'www.ezding.com.tw') {
                url = encodeURI(
                  `https://static.oopocket.com/3rd/ezding/stable/index.html??result=ERROR&order_id=&error_code=1`,
                );
              } else {
                url = encodeURI(
                  `https://at-uat-static.oopocket-dev.com/3rd/ezding/latest/index.html??result=ERROR&order_id=&error_code=1`,
                );
              }
              window.location.href = url;
            } else {
    ...
    } else if (use_method === 2 || use_method === 3 || use_method === 5) {
      // 2:LINE PAY 3:遠傳電信帳單 5:aftee
      Helper.axios.fetch(
  ...
  changeData = (fieldId, value) => {
    ...
    // else if (fieldId === 'use_method' && value === 5) {
    //   // AFTEE 付款 init
    //   console.log('- init --> value', fieldId, value);
    //   this.setState((prevState) => ({ UIData: { ...prevState.UIData, [fieldId]: value } }));
    //   Helper.aftee.getAfteeInit(this.state.UIData);
    // }
    else {
      // Default 設定值
      this.setState((prevState) => ({ UIData: { ...prevState.UIData, [fieldId]: value } }));
  ...
  // handleAftee = () => {
  //   console.log('--->handleAftee');
  //   this.setState(
  //     {
  //       isLoading: true,
  //     },
  //     () => {
  //       Helper.aftee.sendOrder(this.checkData, () => this.setState({ isLoading: false }));
  //     },
  //   );
  // };
    ...
    return (
      <div>
        {isLoading || isLoading2 ? (
          ...
          <div>
            <CheckoutUI useTool={useTool} UIData={UIData} changeData={this.changeData} />
            {/* 信用卡, LINE, 遠傳 結帳 */}
            ...
            {/* AFTEE 結帳 */}
            {/* <div style={style} id={UIData.use_method === 5 ? 'checkOutBtn' : ''} onClick={this.handleAftee}>
              AFTEE
            </div> */}
            <div style={style} id={UIData.use_method === 5 ? 'checkOutBtn' : ''} onClick={this.checkData}>
              AFTEE
            </div>
          </div>
        )}
