...
class HeadMeta extends React.Component {
  render() {
    ...
    if (this.props.moviepage && this.props.movieInfo) {
      ...
      tagstImage = defaultImg;
    } else if (this.props.inviteFriends) {
      // inviteFriends
      title = 'ez訂 | 邀請好友賺優惠，看電影66折劵一起拿';
      tags = ['ez訂-全台最大電影訂票平台', 'ezding', '邀請好友'];
      tagstImage = defaultImg;
    } else if (this.props.inviteFromFriend) {
      // inviteFromFriend
      title = 'ez訂 | 歡迎加入ez訂，領取看電影66折劵2張';
      tags = ['ez訂-全台最大電影訂票平台', 'ezding', '歡迎加入ez訂'];
      tagstImage = defaultImg;
    } else {
      title = 'ez訂-線上訂票平台';
      tags = ['ez訂-全台最大電影訂票平台', 'ezding'];
    ...
    return (
      <div>
        <Head>
                ...
                __html: `
                      {
                          ...
                          "datePublished" : "${timeFormat.newsDate(this.props.articleInfo.result.created_time)}",
          ...
          />
          {/* 測試 */}
          <script src="https://ct-auth.np-pay.com/v1/aftee.js" />
          {/* <script src="https://auth.aftee.tw/v1/aftee.js" /> */}
          <style JSX>
            {`
      			html,body{
              outline:none;
